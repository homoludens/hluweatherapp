...
class NotificationDeviceTest {
    ...
    fun published_summary_contains_a_tap_action() {
        ...
        withNotificationDeliveryPermission {
    ...
    @Test
    fun notification_delivery_permission_is_controlled_and_restored() {
        ...
        assertEquals(wasGranted, hasPostNotificationsPermission())
    }
    ...
    fun denied_notification_permission_can_be_restored_for_delivery() {
        ...
        try {
            ...
            restorePostNotificationsPermission(wasGranted, testFailure)
        }
    }
    ...
    private fun withNotificationDeliveryPermission(block: () -> Unit) {
        ...
        try {
            setPostNotificationsPermission(granted = true)
            assertTrue(AndroidNotificationPermissionChecker.isGranted(context))
            block()
        } catch (error: Throwable) {
            testFailure = error
            throw error
    ...
    private fun restorePostNotificationsPermission(wasGranted: Boolean, testFailure: Throwable?) {
        val restorationFailure = runCatching {
            setPostNotificationsPermission(granted = wasGranted)
            check(hasPostNotificationsPermission() == wasGranted) {
                "Notification permission was not restored to $wasGranted"
            }
        }.exceptionOrNull()
        if (restorationFailure != null) {
            testFailure?.addSuppressed(restorationFailure) ?: throw restorationFailure
        }
    }
    ...
    private fun setPostNotificationsPermission(granted: Boolean) {
        ...
        check(waitForPostNotificationsPermission(granted)) {
            "Permission command did not set POST_NOTIFICATIONS to $granted: ${output.trim()}"
        }
    }

    private fun waitForPostNotificationsPermission(granted: Boolean): Boolean {
        val deadline = SystemClock.elapsedRealtime() + 5_000
        do {
            if (hasPostNotificationsPermission() == granted) return true

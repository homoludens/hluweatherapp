...
Request `weatherapi/locationforecast/2.0/compact` with `lat`, `lon`, and optional `altitude`. Map instant details and `next_1_hours`; unavailable apparent temperature/probability become null. Reject an empty or malformed time series with `WeatherRepositoryException`.
